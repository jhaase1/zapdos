...
XenonGas::XenonGas(const InputParameters & parameters)
  : Material(parameters),
...
void
XenonGas::computeQpProperties()
{
  _massem[_qp] = 9.11e-31;
  _massGas[_qp] = 2.1801714E-25;
  _massArp[_qp] = 2.1801714E-25;
  ...
  if (_interp_trans_coeffs)
  {
    ...
    {
      _muem[_qp] = (std::tanh(_t / (_t_units * 1e-6)) *
                        _mu_interpolation.sample(std::exp(_mean_en[_qp] - _em[_qp])) +
                    (1. - std::tanh(_t / (_t_units * 1e-6))) * .0352) *
                   _voltage_scaling * ((_r_units * _r_units) / _t_units);
      _d_muem_d_actual_mean_en[_qp] =
          std::tanh(_t / (_t_units * 1e-6)) *
          _mu_interpolation.sampleDerivative(std::exp(_mean_en[_qp] - _em[_qp])) *
          _voltage_scaling * ((_r_units * _r_units) / _t_units);
      _diffem[_qp] =
          std::tanh(_t / (_t_units * 1e-6)) *
              _diff_interpolation.sample(std::exp(_mean_en[_qp] - _em[_qp])) +
          (1. - std::tanh(_t / (_t_units * 1e-6))) * .30 * ((_r_units * _r_units) / _t_units);
      _d_diffem_d_actual_mean_en[_qp] =
          std::tanh(_t / (_t_units * 1e-6)) *
          _diff_interpolation.sampleDerivative(std::exp(_mean_en[_qp] - _em[_qp])) *
          ((_r_units * _r_units) / _t_units);
    }
    else
    {
  ...
  else
  {
    ...
    _muem[_qp] =
        0.0352103411399 * _voltage_scaling *
  ...
  // From:
  // Hackam, R. (1969). Mobility and diffusion cross section of atomic xenon ion in xenon. Journal of Physics B: Atomic and Molecular Physics, 2(7), 790. Retrieved from http://stacks.iop.org/0022-3700/2/i=7/a=309
  _muArp[_qp] = 0.56E-4  * _voltage_scaling * ((_r_units * _r_units) / _t_units); // units of m^2/(kV*s) if _voltage_scaling = 1000
  _diffArp[_qp] = _muArp[_qp] * ( _k_boltz[_qp] / ( _e[_qp] * _voltage_scaling )) * _T_gas[_qp];
  ...
  _d_iz_d_actual_mean_en[_qp] =
      _alpha_interpolation.sampleDerivative(std::exp(_mean_en[_qp] - _em[_qp])) / _r_units;
  // _d_iz_d_actual_mean_en[_qp] =
  ...
  _d_ex_d_actual_mean_en[_qp] =
      _alphaEx_interpolation.sampleDerivative(std::exp(_mean_en[_qp] - _em[_qp])) / _r_units;
  ...
  if (_interp_elastic_coeff)
  {
    ...
    _d_el_d_actual_mean_en[_qp] =
        _alphaEl_interpolation.sampleDerivative(std::exp(_mean_en[_qp] - _em[_qp])) / _r_units;
  }
  else
  {
  ...
  _N_A[_qp] = 6.02e23;
  _Ar[_qp] = _p_gas[_qp] / (_k_boltz[_qp] * _T_gas[_qp]);
  _Eiz[_qp] = 12.12;
  _Eex[_qp] = 8.31;
  ...
  //	 {
  //	 _muem[_qp] = (std::tanh(_t / (_t_units * 1e-6)) *
  //_mu_interpolation.sample(std::exp(_mean_en[_qp]-_em[_qp])) + (1. - std::tanh(_t / (_t_units *
  // 1e-6))) * .0352) * _voltage_scaling;
  //	 _d_muem_d_actual_mean_en[_qp] = std::tanh(_t / (_t_units * 1e-6)) *
  //_mu_interpolation.sampleDerivative(std::exp(_mean_en[_qp]-_em[_qp])) * _voltage_scaling;
  //	 _diffem[_qp] = std::tanh(_t / (_t_units * 1e-6)) *
  //_diff_interpolation.sample(std::exp(_mean_en[_qp]-_em[_qp])) + (1. - std::tanh(_t / (_t_units *
  // 1e-6))) * .30;
  //	 _d_diffem_d_actual_mean_en[_qp] = std::tanh(_t / (_t_units * 1e-6)) *
  //_diff_interpolation.sampleDerivative(std::exp(_mean_en[_qp]-_em[_qp]));
  //	 }
  //	 else
  //	 {
  ...
  //	 {
  //	 // From bolos at atmospheric pressure and an EField of 2e5 V/m
  //	 _muem[_qp] = 0.0352103411399 * _voltage_scaling; // units of m^2/(kV*s) if _voltage_scaling =
  // 1000
  //	 _d_muem_d_actual_mean_en[_qp] = 0.0;
