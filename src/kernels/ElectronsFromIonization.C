...
{
  Real electron_flux_mag = (-_muem[_qp] * -_grad_potential[_qp] * std::exp(_u[_qp]) -
                            _diffem[_qp] * std::exp(_u[_qp]) * _grad_u[_qp])
                               .norm();
...
ElectronsFromIonization::computeQpJacobian()
{
  if (_var.number() == _em_id)
  {
    ...
    RealVectorValue electron_flux = -_muem[_qp] * -_grad_potential[_qp] * std::exp(_u[_qp]) -
                                    _diffem[_qp] * std::exp(_u[_qp]) * _grad_u[_qp];
    RealVectorValue d_electron_flux_d_em =
        -d_muem_d_em * -_grad_potential[_qp] * std::exp(_u[_qp]) -
        _muem[_qp] * -_grad_potential[_qp] * std::exp(_u[_qp]) * _phi[_j][_qp] -
        d_diffem_d_em * std::exp(_u[_qp]) * _grad_u[_qp] -
...
ElectronsFromIonization::computeQpOffDiagJacobian(unsigned int jvar)
{
  ...
  Real electron_flux_mag = electron_flux.norm();
	...
	Real d_iz_term_d_potential;
	Real d_iz_d_mean_en;
	RealVectorValue d_electron_flux_d_mean_en;
	Real d_electron_flux_mag_d_mean_en;
	Real d_iz_term_d_mean_en;
	Real d_iz_d_em;
	RealVectorValue d_electron_flux_d_em;
	Real d_electron_flux_mag_d_em;
	Real d_iz_term_d_em;
	
  if (jvar == _potential_id)
	{
		d_electron_flux_d_potential =
      -_muem[_qp] * -_grad_phi[_j][_qp] * std::exp(_u[_qp]);
			
	  d_electron_flux_mag_d_potential =
      electron_flux * d_electron_flux_d_potential /
      (electron_flux_mag + std::numeric_limits<double>::epsilon());

		d_iz_term_d_potential = (_alpha_iz[_qp] * d_electron_flux_mag_d_potential);
		
    return -_test[_i][_qp] * d_iz_term_d_potential;
	}
  else if (jvar == _mean_en_id)
	{
		d_iz_d_mean_en = _d_iz_d_actual_mean_en[_qp] * actual_mean_en * _phi[_j][_qp];
	
		d_electron_flux_d_mean_en =
				-d_muem_d_mean_en * -_grad_potential[_qp] * std::exp(_u[_qp]) -
				d_diffem_d_mean_en * std::exp(_u[_qp]) * _grad_u[_qp];
				
		d_electron_flux_mag_d_mean_en = electron_flux * d_electron_flux_d_mean_en /
                                       (electron_flux_mag + std::numeric_limits<double>::epsilon());
	...
	{
		d_iz_d_em = _d_iz_d_actual_mean_en[_qp] * actual_mean_en * -_phi[_j][_qp];
	
		d_electron_flux_d_em =
        -d_muem_d_em * -_grad_potential[_qp] * std::exp(_u[_qp]) -
        _muem[_qp] * -_grad_potential[_qp] * std::exp(_u[_qp]) * _phi[_j][_qp] -
        d_diffem_d_em * std::exp(_u[_qp]) * _grad_u[_qp] -
        _diffem[_qp] * std::exp(_u[_qp]) * _phi[_j][_qp] * _grad_u[_qp] -
		...
		d_electron_flux_mag_d_em = electron_flux * d_electron_flux_d_em /
				(electron_flux_mag + std::numeric_limits<double>::epsilon());
